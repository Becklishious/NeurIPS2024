...
def discriminative_score_metrics (ori_data, generated_data):
  ...
  # Basic Parameters
  no, seq_len, dim = np.asarray(ori_data).shape    #获取原始数据的形状信息
    
  # Set maximum sequence length and each sequence length
  # 获取原始数据和生成数据的时间信息和最大序列长度
  ...
  # 将原始数据和生成数据的输出合并，并生成相应的标签。  
  y_pred_final = np.squeeze(np.concatenate((y_pred_real_curr, y_pred_fake_curr), axis = 0))
  y_label_final = np.concatenate((np.ones([len(y_pred_real_curr),]), np.zeros([len(y_pred_fake_curr),])), axis = 0)
    
  """
  (y_pred_final > 0.5): 通过比较判别器的输出是否大于 0.5 生成了对应的预测标签，
  其中大于 0.5 被视为真实数据 否则为生成数据。
  """
  # Compute the accuracy
  acc = accuracy_score(y_label_final, (y_pred_final>0.5)) #正确分类的样本/全部样本
